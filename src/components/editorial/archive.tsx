...
import { StoryCard, StoryListItem } from "./story-card";
  ...
  return (
    <section className="px-4 py-10 sm:px-6 lg:px-10 lg:py-14">
      <div className="mx-auto max-w-[1220px]">
        ...
        <Pagination page={result.page} totalPages={result.totalPages} query={query} basePath={basePath} />
      </div>
    </section>
  );
}

export function Pagination({ page, totalPages, query, basePath }: { page: number; totalPages: number; query?: Record<string, string>; basePath?: string }) {
  ...
  return (
    <nav className="mt-14 flex flex-wrap items-center justify-center gap-2" aria-label="Paginação">
      <PageLink page={page - 1} disabled={page <= 1} label="Anterior" query={query} basePath={basePath} />
      {pages.map((item, index) => (
        <span key={item} className="contents">
          {index > 0 && item - pages[index - 1] > 1 ? <span className="px-1 text-muted" aria-hidden>…</span> : null}
          <PageLink page={item} current={item === page} label={String(item)} query={query} basePath={basePath} />
        </span>
      ))}
      <PageLink page={page + 1} disabled={page >= totalPages} label="Próxima" query={query} basePath={basePath} />
    </nav>
  );
}

function PageLink({ page, label, current = false, disabled = false, query, basePath }: { page: number; label: string; current?: boolean; disabled?: boolean; query?: Record<string, string>; basePath?: string }) {
  ...
  // A document navigation keeps query-dependent canonical/title metadata in
  // sync on Next 16.3; client-only query transitions can retain the old head.
  return <a href={getPaginationHref(page, query, basePath)} className={className} aria-current={current ? "page" : undefined}>{label}</a>;
}
